...
james::background* background_{ nullptr };
james::terrain* terrain_{nullptr};
james::player* player_{ nullptr };
james::interface interface_;
james::collision_detection collision_detection_;
...
*/
  ...
  switch (event_type) {
      ...
      case EMSCRIPTEN_EVENT_DBLCLICK:   {
			if (player_->get_completed()) { player_->hard_reset(); terrain_->reinitialize(); }
			else if (player_->get_dead()) { player_->soft_reset(); }
      }
  ...
  switch (event_type) {
      ...
      case EMSCRIPTEN_EVENT_KEYDOWN:  {
			//std::cout << e->code << std::endl;
				if (*(e->key) == 'a') {
					interface_.keya_ = true;
				}
				if (*(e->key) == 's') {
					interface_.keys_ = true;
				}
				if (*(e->key) == 'd') {
					interface_.keyd_ = true;
				}
				if (*(e->key) == 'w') {
					interface_.keyw_ = true;
				}
				if (std::string(e->code) == "ArrowLeft") {
					interface_.keya_ = true;
				}
				if (std::string(e->code) == "ArrowDown") {
					interface_.keys_ = true;
				}
				if (std::string(e->code) == "ArrowRight") {
					interface_.keyd_ = true;
				}
				if (std::string(e->code) == "ArrowUp") {
					interface_.keyw_ = true;
				}
        break;
      }
      case EMSCRIPTEN_EVENT_KEYUP:    {
				if (*(e->key) == 'a') {
					interface_.keya_ = false;
				}
				if (*(e->key) == 's') {
					interface_.keys_ = false;
				}
				if (*(e->key) == 'd') {
					interface_.keyd_ = false;
				}
				if (*(e->key) == 'w') {
					interface_.keyw_ = false;
				}
				if (std::string(e->code) == "ArrowLeft") {
					interface_.keya_ = false;
				}
...
void generate_frame() {
	...
	// draw
	background_->display(0, 0, (player_->get_xpos()-180.0) / (148.0-3.0) / 60.0, ((148.0+3.0+0.5)*60.0-player_->get_xpos())/pixel_width_, (180.0+30.0)/pixel_width_, player_->get_ypos() / pixel_height_);
	terrain_->display(-player_->get_xpos() + 180, 0);
	player_->display();
...
int main(int argc, char *argv[])
{
  ...
  font_.load_font("font-data-40.data", nullptr);
   ...
   background_ = new james::background();
	background_->load(nullptr, pixel_width_, pixel_height_);
  
	terrain_ = new james::terrain();
	terrain_->load("terrain_atlas.png");

	collision_detection_.set_terrain(terrain_);

	player_ = new james::player(interface_, collision_detection_);
	player_->load("player.png");
